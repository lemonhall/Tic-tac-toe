# 井字棋决斗场 (Tic-Tac-Toe Arena)

一个功能完整的井字棋游戏平台，支持人机对战、AI对战和观战模式。

## 🎮 功能特点

### 核心功能
- ✅ **人类 vs 人类**: 两个玩家通过Web界面对战
- ✅ **人类 vs AI**: 与内置AI对战
- ✅ **AI vs AI**: 观看两个AI相互对弈
- ✅ **观战模式**: 纯观看模式，不参与游戏
- ✅ **实时更新**: 使用SSE (Server-Sent Events) 实时推送游戏状态
- ✅ **外部Agent接入**: 支持外部AI Agent接入进行对战

### 技术特点
- 🎨 **模块化前端**: HTML + 模块化CSS + 模块化JavaScript
- 🐍 **Python后端**: Flask + RESTful API + SSE
- 🤖 **智能AI**: 基于Minimax算法的AI对手
- 📊 **统计功能**: 记录历史战绩和移动历史
- 🎯 **规则验证**: 完整的游戏规则和非法移动检测

## 📁 项目结构

```
Tic-tac-toe/
├── index.html              # 主页面
├── static/
│   ├── css/               # 模块化CSS
│   │   ├── reset.css      # 样式重置
│   │   ├── layout.css     # 布局样式
│   │   ├── board.css      # 棋盘样式
│   │   ├── controls.css   # 控制面板样式
│   │   ├── status.css     # 状态显示样式
│   │   └── modal.css      # 模态框样式
│   └── js/                # 模块化JavaScript
│       ├── main.js        # 入口文件
│       ├── api.js         # API客户端
│       ├── gameState.js   # 游戏状态管理
│       ├── ui.js          # UI管理器
│       ├── events.js      # 事件处理
│       └── gameController.js  # 游戏控制器
├── app.py                 # Flask服务器
├── game_logic.py          # 游戏核心逻辑
├── ai_strategy.py         # AI策略实现
├── game_manager.py        # 游戏管理器
└── requirements.txt       # Python依赖
```

## 🚀 快速开始

### 1. 安装依赖

```bash
pip install -r requirements.txt
```

### 2. 启动服务器

```bash
python app.py
```

### 3. 访问游戏

打开浏览器访问: `http://localhost:5000`

## 🎯 使用说明

### 游戏模式

1. **人类 vs 人类**: 两个玩家轮流在同一设备上点击格子下棋
2. **人类 vs AI**: 你对抗内置AI
3. **AI vs AI**: 观看两个AI对战
4. **观战模式**: 纯观看模式

### 操作步骤

1. 选择游戏模式
2. 点击"开始游戏"
3. 在棋盘上点击空白格子下棋
4. 游戏结束后可以选择"再来一局"

## 🔌 API接口

### RESTful API

#### 创建游戏
```http
POST /api/game/create
Content-Type: application/json

{
  "player_x_type": "human",  // human, ai, agent
  "player_o_type": "ai"
}
```

#### 获取游戏状态
```http
GET /api/game/{game_id}/state
```

#### 下棋
```http
POST /api/game/{game_id}/move
Content-Type: application/json

{
  "row": 0,
  "col": 0,
  "player_id": null  // 可选
}
```

#### AI移动
```http
POST /api/game/{game_id}/ai-move
```

#### 重置游戏
```http
POST /api/game/{game_id}/reset
```

#### SSE事件流
```http
GET /api/game/{game_id}/events
```

### SSE事件类型

- `connected`: 连接建立
- `move`: 玩家移动
- `game_over`: 游戏结束
- `reset`: 游戏重置
- `state_update`: 状态更新

## 🤖 AI策略

### SimpleAI (默认)
基于规则的AI，策略优先级：
1. 如果能赢，立即获胜
2. 如果对手能赢，阻止对手
3. 占据中心
4. 占据角落
5. 占据边缘

### TicTacToeAI (高级)
使用Minimax算法with Alpha-Beta剪枝
- 支持三个难度级别: easy, medium, hard
- 完美策略，几乎不会输

## 🔧 外部Agent接入

### 通过API接入

外部AI Agent可以通过以下方式接入：

1. 创建游戏时指定玩家类型为 `agent`
2. 使用SSE监听游戏事件
3. 当轮到Agent时，调用 `/api/game/{game_id}/move` 接口

### 示例代码 (Python)

```python
import requests
import json

# 创建游戏
response = requests.post('http://localhost:5000/api/game/create', 
    json={'player_x_type': 'agent', 'player_o_type': 'ai'})
game_id = response.json()['game_id']

# 监听事件
def listen_events():
    url = f'http://localhost:5000/api/game/{game_id}/events'
    with requests.get(url, stream=True) as r:
        for line in r.iter_lines():
            if line.startswith(b'data:'):
                event = json.loads(line[6:])
                if event['type'] == 'move' and event.get('next_player') == 'X':
                    # 轮到我了，下棋
                    make_move(game_id, row, col)

def make_move(game_id, row, col):
    requests.post(f'http://localhost:5000/api/game/{game_id}/move',
        json={'row': row, 'col': col})
```

## 🎨 自定义样式

所有CSS都是模块化的，可以轻松自定义：

- `reset.css`: 基础样式重置
- `layout.css`: 整体布局
- `board.css`: 棋盘外观
- `controls.css`: 按钮和控件
- `status.css`: 状态显示
- `modal.css`: 弹窗样式

## 📊 游戏规则

1. 棋盘为3x3网格
2. X先手，O后手
3. 玩家轮流在空格中放置标记
4. 率先在横、竖或斜线上形成连续三个标记的玩家获胜
5. 如果棋盘填满仍无人获胜，则平局

## 🔮 未来扩展

系统设计支持以下扩展：

- ✅ 接入基于LLM的AI Agent
- ✅ 接入基于强化学习(RL)的AI系统
- ✅ 两个外部Agent相互对弈
- ✅ 多人观战
- ⏳ 游戏回放功能
- ⏳ 排行榜系统
- ⏳ 锦标赛模式

## 📝 开发说明

### 前端模块

- `api.js`: 封装所有API调用和SSE连接
- `gameState.js`: 管理客户端游戏状态
- `ui.js`: 处理所有UI渲染和更新
- `events.js`: 处理用户交互事件
- `gameController.js`: 协调各模块，主控制器

### 后端模块

- `game_logic.py`: 游戏核心逻辑和规则
- `ai_strategy.py`: AI策略实现
- `game_manager.py`: 管理多个游戏实例
- `app.py`: Flask服务器和API端点

## 🐛 故障排除

### SSE连接问题
- 确保浏览器支持EventSource
- 检查防火墙设置
- 查看浏览器控制台错误信息

### AI不响应
- 检查服务器日志
- 确认游戏状态正确
- 验证玩家类型设置

## 📄 许可证

MIT License

## 👥 贡献

欢迎提交Issue和Pull Request！

---

**井字棋决斗场** - 一个为AI Agent设计的对战平台 🎮
